...
- 그림·예시 형식은 `docs/contributing/doc-style.md`를 따른다. 현재 동작 예시는 실제로 실행한 출력에서 옮긴다. `$` 명령과 출력을 싣는 예시에는 블록 바로 앞에 `<!-- agctx-example: <고정물> -->`를 달고 `evals/doc-scenarios.test.ts`에 고정물을 둔다. 그러면 출력 문구 때문에 메시지 카탈로그를 핀하지 않아도 된다.
