...
- **저장소 성격:** 이 저장소는 `@isthis/agentic` npm 패키지의 개발·배포 원본 저장소다.
...
- `Proposed`·`Implementing` 논의 문서는 상태 아래에 대상 계층, 중요도와 근거, 선행·후속 작업, 권장 다음 작업을 담은 **제안 요약**과 주요 논점을 보여 주는 **목차**를 둔다. 중요도는 확정된 로드맵이 아니라 현재 위험과 의존성에 따른 논의 우선순위다. 세부 형식은 `docs/discussion/architecture/topics/implementation-contracts.md`를 따른다.
