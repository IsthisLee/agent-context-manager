...
1. **말보다 실제 검증 실행 결과가 우선이다.**
  ...
  - 검증 결과는 마지막에 한 줄로 간결하게 보고하라. 예: `[ @agentic: 프로젝트 검증 통과(pnpm run check) — <실행한 검사와 통과 수>. ]` 실제 실행한 검사와 통과 수를 기재하고, 경고·한계가 있으면 문장 끝에 덧붙인다.
...
- **기능 인터페이스 동등성:** 새 사용자 기능은 CLI 명령·옵션, TUI 흐름, `agt profile list`의 프로필 관리 메뉴에서 모두 실행 가능해야 한다. 전역 도움말이나 저장소 개발 전용 기능처럼 특정 프로필에 귀속되지 않는 기능만 예외로 둔다. 공통 기능 registry에 세 경로를 등록하고, 각 경로의 동작 평가를 추가하라.
