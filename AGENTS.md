...
- **논의 문서는 그림으로 먼저 보여 준다.** `docs/`에 문서를 새로 쓰거나 수정, 논점을 추가할 때, 구성 요소의 경계·처리 흐름·상태 변화·데이터 관계처럼 글로만 따라가기 어려운 내용은 그림을 적극적으로 활용하여 설명하라. 형식은 내용에 맞춰 고른다: 구성 요소와 책임 경계는 아키텍처 다이어그램, 명령·동기화 절차는 플로차트나 시퀀스 다이어그램, 상태 전이는 상태 다이어그램, 파일·설정·프로필 사이의 관계는 ERD, 대안 비교는 표를 쓴다.
  - 기본 형식은 GitHub에서 바로 렌더링되고 diff로 리뷰할 수 있는 Mermaid 코드 블록이다. archify 같은 도구로 이미지를 만들었다면 그 이미지를 생성한 원본 소스를 같은 변경에 함께 커밋하고 원본 없이 이미지만 커밋하지 마라.
  - 그림은 본문을 대체하지 않는다. 그림 바로 아래에 그림이 보여 주는 대상과 핵심 결론을 한두 문장으로 적고 논의가 바뀌면 그림과 본문을 같은 변경에서 함께 고쳐라.
  - 한 문장으로 충분히 설명되는 내용에 장식용 그림을 넣지 마라.
