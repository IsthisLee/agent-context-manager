...
- 프로필이 MCP 서버를 담는다. 프로필 폴더의 `mcp.json`에 `command`·`args`·`env`(로컬 서버)나 `url`·`headers`(원격 서버)로 서버를 적으면, `profile apply`·`sync`가 Claude Code의 `.mcp.json`과 Codex의 `.codex/config.toml`에 옮겨 쓴다. agctx가 쓴 서버만 바꾸고 사람이 넣은 서버와 설정은 그대로 두며, 사람이 같은 이름의 서버를 두었으면 멈춘다. 적용 계획에는 쓸 서버의 명령과 URL이 나온다. Codex는 `${VAR}`를 펼치지 않으므로 같은 이름의 환경 변수는 `env_vars`·`env_http_headers`·`bearer_token_env_var`로 옮기고, 옮길 수 없는 서버는 Codex에서만 빼고 경고한다. Codex가 사용자 설정과 키 하나씩 합치므로, 이 컴퓨터의 Codex 사용자 설정에 같은 이름의 서버가 있으면 경고한다. `profile apply --include rules`는 MCP를 받지 않고 그 선택을 `agctx.project.json`의 `include`에 남긴다. Antigravity에는 아직 쓰지 않는다. 근거는 [ADR 0044](docs/adr/0044-mcp-servers-in-profiles.md)
- `profile apply --agent codex,claude`로 저장소마다 파일을 받을 에이전트를 고른다. 고른 목록은 `agctx.project.json`의 `agents`에 남아 `profile sync`·`repos sync`·`repos pr`·`check`가 같은 선택을 따르고, `--agent all`은 기록을 지워 지원 에이전트 전부로 되돌린다. 지금까지는 쓰지 않는 에이전트의 파일(`.agents/rules/agctx.md` 등)도 늘 만들어졌고 지워도 다음 `sync`가 다시 만들었다. `AGENTS.md`는 모든 에이전트가 읽으므로 늘 쓴다. 뺀 에이전트의 파일은 관리 블록만 지우고, 사람이 쓴 내용이 없으면 파일과 `.agctx/base/` 사본도 지운다. `explain`은 고르지 않은 에이전트를 `not-selected`로 표시하고 그 누락을 종료 코드 4로 세지 않으며, `verify`는 `--agent`로 이름을 대지 않으면 확인하지 않는다. TUI의 「프로젝트에 적용」은 기록한 선택을 미리 체크한 목록을 보여 준다. 근거는 [ADR 0042](docs/adr/0042-choose-agents-per-repository.md)
- `profile apply`·`profile sync`·`repos sync`가 쓸 프로젝트 `AGENTS.md`가 200줄을 넘거나 24 KiB 이상이면 경고한다. Claude Code는 지침 파일마다 200줄 미만을 권하고, Codex는 지침 파일을 합쳐 32 KiB에 이르면 더 읽지 않는다. 지금까지는 파일이 길어져 뒤쪽 규칙이 빠져도 알려 주지 않았다. 경고만 하고 파일과 종료 코드는 바꾸지 않으며, `--json`이면 `warnings`에 담긴다. 근거는 [ADR 0041](docs/adr/0041-warn-on-long-project-agents-md.md)
- 이미 컴퓨터에 받아 둔 규칙 저장소 폴더를 `agctx profile link [<폴더>]`로 프로필로 연결한다. 폴더에 `profile.json`이 없으면 규칙 파일을 찾아 만들고(여럿이면 `--instructions`로 고른다), 보관함에는 그 폴더를 가리키는 `link.json`만 둔다. 커밋이나 push는 하지 않는다. 연결한 프로필은 폴더를 직접 읽으므로 커밋하기 전의 수정도 바로 `apply`에 들어가고, `uncommitted`로 기록되며 `--pin`은 거부된다. 지금까지는 `profile.json`을 손으로 써서 커밋·push한 뒤 `clone`해야 했고, 시험하려 해도 브랜치를 따야 했다. 연결한 프로필에서 `pull`·`push`·`connect`는 그 폴더에서 git으로 하라고 안내하며 멈추고, `remove`는 포인터만 지운다. `status`는 `--refresh`를 줘도 그 폴더에서 `fetch`하지 않는다. 폴더를 옮기거나 그 폴더의 `profile.json`이나 규칙 파일이 없어지면 `profile list`가 끊긴 링크로 이유와 함께 보여 주고(`--json`이면 `brokenLinks`), `check`는 멈추지 않고 경고한다. 끊긴 링크는 `profile remove` 뒤 다시 `link`해 되살리고, 안내가 연결할 때의 이름·용도·규칙 파일을 채운 명령을 알려 준다. `link`는 링크를 다른 폴더로 옮기지 않고, 한 폴더에는 링크 하나만 둔다. Git 저장소 안의 하위 폴더는 연결하지 않고, 저장소 루트를 연결하면서 규칙 파일을 `--instructions`로 주는 명령을 알려 준다. 홈 폴더는 연결하지 않는다. `repos status`는 `check`의 경고를 저장소 줄 아래에 보여 주고 `--json`이면 `warnings`에 담는다. TUI는 첫 화면과 프로필 목록의 「폴더를 프로필로 연결」이다. 근거는 [ADR 0037](docs/adr/0037-link-existing-folder-as-profile.md)
- 에이전트용 스킬이 CLI 패키지에 들어가고, `agctx install`이 이 컴퓨터에 설치된 Claude Code(`~/.claude/skills/`)·Codex(`~/.agents/skills/`)·Antigravity(앱·IDE `~/.gemini/config/skills/`, CLI `~/.gemini/antigravity-cli/skills/`)의 스킬 폴더에 복사한다. 폴더마다 설치 기록 `.agctx-install.json`을 두고, 기록과 같은 폴더만 바꾸며, 기록이 없거나 고친 폴더는 `--force` 없이는 건드리지 않는다. `agctx uninstall`은 자기가 둔 폴더만 지운다. TUI 첫 화면의 「에이전트 스킬 설치」·「제거」로도 실행한다. 설치된 스킬이 CLI와 버전이 다르면 모든 명령이 한 줄로 알린다. 근거는 [ADR 0038](docs/adr/0038-install-agent-skills-from-cli-package.md)
...
- 진단용 에이전트 스킬 `agctx`도 사용자가 `/agctx`로 부를 때만 쓰인다. 지금까지는 에이전트가 "지침이 최신인지 봐 줘" 같은 대화를 보고 스스로 불러 `check`·`explain`·`verify` 등을 실행할 수 있었다. Claude Code는 `disable-model-invocation: true`, Codex는 `agents/openai.yaml`의 `allow_implicit_invocation: false`로 막고, 자동 호출을 끄는 설정이 없는 Antigravity에는 스킬 본문에 같은 규칙을 적었다. `agctx-author`는 이미 이름으로 부를 때만 쓰였다. 이미 설치한 스킬은 `agctx install`을 다시 실행하면 바뀐다. 근거는 [ADR 0047](docs/adr/0047-agent-skills-explicit-invocation-only.md)
- 사람이 쓴 `AGENTS.md`·`CLAUDE.md`·`.agents/rules/agctx.md`(agctx가 쓴 적 없고 agctx 표지도 없는 파일)가 이미 있으면 `profile apply`·`profile sync`가 아무것도 쓰지 않고 종료 코드 2로 멈춘다. 지금까지는 확인 없이 `CLAUDE.md` 뒤에 관리 블록을 붙이고 `AGENTS.md`의 내용을 프로필 지침 아래로 옮겼다. `--adopt`를 붙이면 지금까지처럼 기존 내용을 남기고 관리 영역을 더하며, 한 번 편입한 파일은 다음부터 묻지 않는다. TUI는 멈춘 자리에서 편입할지 묻는다. **호환성:** 이미 `AGENTS.md`나 `CLAUDE.md`가 있는 저장소에 처음 적용하는 스크립트는 `--adopt`를 더해야 한다. agctx로 이미 적용한 저장소는 달라지지 않지만, `agctx.project.json`의 `agents`에 에이전트를 더했는데 그 에이전트의 파일을 사람이 이미 써 두었다면 거기서 멈춘다. `profile resolve`도 `--adopt`를 받고, `check`는 그런 파일을 충돌(2)로 보고한다. `--json` 오류는 멈춘 파일을 `details`에 담는다. 근거는 [ADR 0043](docs/adr/0043-stop-on-unmanaged-files.md)
- 기본 설치 안내가 `npm install -g agent-context-manager`와 `agctx install` 두 줄로 바뀐다. 지금까지는 스킬을 skills CLI(`npx skills add ... -g -a claude-code -a codex -a antigravity`)로 GitHub `main`에서 받아, 설치한 CLI와 스킬의 명령 목록이 어긋날 수 있었고 Antigravity의 전역 스킬 위치에는 설치되지 않았다. 전에 skills CLI로 설치했다면 처음 한 번 `agctx install --force`를 실행한다
...
- 파일을 다시 쓸 때 기존 파일의 권한 모드와 줄 끝을 읽는 곳, 그리고 `CLAUDE.md`가 `AGENTS.md`를 가져오는지 판정하는 곳이 경로로 파일을 확인한 뒤 같은 경로를 다시 읽었다. 확인과 읽기 사이에 파일이 바뀌면 다른 파일의 내용으로 판단할 수 있었다. 이제 파일을 한 번 열고 그 파일에서 확인하고 읽는다. CodeQL `js/file-system-race` 경고 두 곳이다
