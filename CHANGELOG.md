...
- `profile apply`·`profile sync`·`repos sync`가 쓸 프로젝트 `AGENTS.md`가 200줄을 넘거나 24 KiB 이상이면 경고한다. Claude Code는 지침 파일마다 200줄 미만을 권하고, Codex는 지침 파일을 합쳐 32 KiB에 이르면 더 읽지 않는다. 지금까지는 파일이 길어져 뒤쪽 규칙이 빠져도 알려 주지 않았다. 경고만 하고 파일과 종료 코드는 바꾸지 않으며, `--json`이면 `warnings`에 담긴다. 근거는 [ADR 0024](docs/adr/0024-guidance-evidence-and-budget.md)
