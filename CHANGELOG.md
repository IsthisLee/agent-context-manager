...
- 이미 컴퓨터에 받아 둔 규칙 저장소 폴더를 `agctx profile link [<폴더>]`로 프로필로 연결한다. 폴더에 `profile.json`이 없으면 규칙 파일을 찾아 만들고(여럿이면 `--instructions`로 고른다), 보관함에는 그 폴더를 가리키는 `link.json`만 둔다. 커밋이나 push는 하지 않는다. 연결한 프로필은 폴더를 직접 읽으므로 커밋하기 전의 수정도 바로 `apply`에 들어가고, `uncommitted`로 기록되며 `--pin`은 거부된다. 지금까지는 `profile.json`을 손으로 써서 커밋·push한 뒤 `clone`해야 했고, 시험하려 해도 브랜치를 따야 했다. 연결한 프로필에서 `pull`·`push`·`connect`는 그 폴더에서 git으로 하라고 안내하며 멈추고, `remove`는 포인터만 지운다. `status`는 `--refresh`를 줘도 그 폴더에서 `fetch`하지 않는다. 폴더를 옮기거나 그 폴더의 `profile.json`이나 규칙 파일이 없어지면 `profile list`가 끊긴 링크로 이유와 함께 보여 주고(`--json`이면 `brokenLinks`), `check`는 멈추지 않고 경고한다. `--name`으로 같은 이름을 주고 다시 `link`하면 이어진다. Git 저장소 안의 하위 폴더는 연결하지 않고, 저장소 루트를 연결하면서 규칙 파일을 `--instructions`로 주는 명령을 알려 준다. TUI는 첫 화면과 프로필 목록의 「폴더를 프로필로 연결」이다. 근거는 [ADR 0037](docs/adr/0037-link-existing-folder-as-profile.md)
