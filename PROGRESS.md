...
- 문단을 정해진 너비로 다시 접는 포매터 설정(Prettier의 `proseWrap: always` 등)을 쓰는 저장소에서는 관리 영역이 통째로 다시 쓰이므로 충돌을 피할 수 없다. 68줄짜리 `AGENTS.md`에서 18줄이 사라지고 100줄이 새로 쓰였다([근거](docs/references.md#포매터가-관리-영역을-바꾸는-범위)). 지금은 적용된 파일을 포맷 대상에서 빼라고 안내할 뿐이고, 해시를 비교하기 전에 정규화할지는 정하지 않았다.
