...
**위치:** `/Users/isthis/Documents/task/agentic`  
**상태:** 설계 문서 배치 및 GitHub 비공개 저장소 초기화 완료 · Core 런타임/에이전트 설정/CI 구축 전
...
`packages/`, `templates/`, `evals/`, `provenance/`는 후속 Core 구현 단계에서 만든다. 의존성 설치는 아직 수행하지 않았다.
...
`agentic-core`는 범용 Core 구성요소의 명칭이며, 실제 환경 폴더는 `agentic`이다. `.agentic/`와 `agentic` 명령은 설계상 규약이며 아직 자동 동작하는 내장 기능이 아니다. 프로젝트 언어/패키지 관리자를 강제하지 않고 Adapter에 실행 명령을 연결한다.
