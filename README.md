...
>
> Agentic은 개인·조직별 에이전틱 개발 지침을 Core로 생성·설정하고, 이를 로컬 또는 Git 기반으로 관리하며 프로젝트와 여러 AI 에이전트에 안전하게 적용·동기화하는 패키지입니다.
...
개인 개발자도 프로젝트별 `Personal` Core를 나누어 재사용하고, 사용하는 AI 도구가 바뀌어도 같은 지침을 유지할 수 있습니다. 반복 설정과 프로젝트 사이의 규칙 드리프트를 줄여 관리와 개발을 더 편하게 만듭니다.
...
> 아래 Git 기반 흐름은 Profile 원격 관리 기능이 구현된 뒤의 사용 기준입니다. 현재 릴리스는 로컬 Core 관리·적용·동기화만 제공합니다.

### 개인 개발

- **이렇게 사용합니다:** 프로젝트 성격별 Personal Profile을 만들고 `setup`으로 지침을 구성한 뒤, 각 프로젝트에 `init`으로 적용합니다.
