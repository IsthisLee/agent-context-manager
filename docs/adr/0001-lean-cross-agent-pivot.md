...
1. 자체 에이전트 런타임과 중앙 오케스트레이터를 구축한다. 벤더 CLI 변화에 대한 유지보수 비용과 런타임 종속성이 커 채택하지 않는다.
2. 특정 벤더의 에이전트 팀 도구만 사용한다. 크로스 에이전트 규칙과 검증 계약을 제공할 수 없어 채택하지 않는다.
3. 공식 런타임 위에서 지침 동기화와 네이티브 검증만 제공한다. 유지보수 경계를 명확히 하면서 여러 도구를 지원할 수 있어 채택한다.
