...
4. **자산 격리 및 서브에이전트 기반 규칙 승격 (Subagent Promotion):**
   * 프로젝트 도메인 코드가 Core로 유출되지 않도록 엄격히 분리하되, 서브에이전트를 통해 기밀이 제거된 순수 기술 패턴만 Core로 승격한다.
...
* **긍정적 영향:**
  ...
  * 회사의 기밀 유출 방지와 개인 오픈소스 자산의 법적 독립성 확보.
* **유보/폐기된 사항:**
