...
1. 동기화 시 모든 지침 파일을 단순 덮어쓴다. 사용자 규칙이 유실되므로 채택하지 않는다.
2. 사용자에게 모든 지침·테스트 설정을 수동으로 만들게 한다. 신규 프로젝트의 검증 시작 비용이 높아 채택하지 않는다.
3. 공통 지침은 갱신하면서 확장 영역을 보존하고, 빈 프로젝트에는 최소 테스트 부트스트랩을 제공한다. 이후 기존 테스트 보존 경계는 ADR 0007에서 강화한다.
