...
## 2. 검토한 대안 (Options Considered)

1. TypeScript 소스와 컴파일 산출물을 패키지에 포함한다. 대상 프로젝트의 런타임 의존성과 빌드 산출물 괴리 위험이 있어 채택하지 않는다.
2. 순수 JavaScript에서 타입 계약을 전혀 관리하지 않는다. 유지보수성과 IDE 지원이 약해져 채택하지 않는다.
3. 의존성 없는 Native ESM에 JSDoc과 `jsconfig.json`을 결합한다. 실행 단순성과 편집기 지원을 함께 확보하므로 채택한다.
