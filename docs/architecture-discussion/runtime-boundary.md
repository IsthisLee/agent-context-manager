...
`agentic analyze`는 읽기 전용 분석만 하고, `agentic plan`은 역할 선택과 artifact 생성만 한다. 가상 팀 선택은 공식 런타임에서 수행할 역할을 선언하는 것이며 Agentic이 런타임을 조정한다는 뜻이 아니다. `check.mjs`는 대상 프로젝트의 표준 검증 계약을 실행할 뿐 런타임 래퍼가 아니다.
