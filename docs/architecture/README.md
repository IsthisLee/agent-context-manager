...
* 생성된 `tools/agentic/check.mjs`를 실행한 경우, 완료 판단에 필요한 명령·종료 코드·소요 시간·성공 여부를 `.agentic/last-check.json`에 기록한다. 기존 `npm run check` 스크립트의 증거 연결은 아직 보장하지 않으며, [검증 논의](../discussion/architecture/verification.md)에서 개선한다. [근거: [Anthropic Evals 연구](../references.md#anthropic-demystifying-evals)]
