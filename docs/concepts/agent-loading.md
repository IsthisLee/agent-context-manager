...
- `missing`: 받아야 할 지침 파일을 그 에이전트가 받지 못한다. 확인한 에이전트(기본은 세 에이전트 모두, `--agent`로 고른다) 가운데 하나라도 `missing`이 있으면 종료 코드 4로 끝난다. 저장소가 `agctx.project.json`에서 고르지 않은 에이전트는 `not-selected`로 표시하고 그 `missing`은 세지 않는다(`src/explain.ts`의 `explainPath`<!--s:cc7f0e0a7855-->).
