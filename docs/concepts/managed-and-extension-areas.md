...
<!-- agctx-doc-sources-sha256: 2b5caea9cefa044c20abbfceb20c9e656764d4b5b0c15f17f185a6fa3125d431 -->
...
## 4. 프로젝트 규칙 확장 (SSOT)                ← 프로필 것 (경계)
이 프로젝트에만 적용되는 도메인 규칙은…          ← 프로필 것 (안내 한 줄)

- 배포 전에 QA를 받는다.                       ← 여기부터 사용자 것
```

경계 아래 안내 한 줄까지 프로필이 소유한다. `AGENTS.md`에는 관리 블록 표지가 없으므로 이 제목과 안내문이 눈에 보이는 유일한 경계 표시이고, 지워도 `sync`가 다시 넣는다.

무엇을 고치면 어떤 판정이 나오는지는 이렇다(2026-09-20 실측).

| 고친 곳 | `agctx check` | 다음에 할 일 |
| --- | --- | --- |
| 경계 아래에 규칙을 더하거나 고침 | 통과(0) | 없음. `sync`도 필요 없다 |
| 경계 제목이나 안내 한 줄을 고치거나 지움 | 뒤처짐(1) | `profile sync`. 제목과 안내문이 돌아오고 사용자가 쓴 규칙은 남는다 |
| 경계 위(프로필 영역)를 고침 | 충돌(2) | `profile resolve`. 고친 내용을 경계 아래로 옮긴다 |
| 편집기가 저장하면서 파일 전체를 다시 포맷 | 통과(0) | 없음. 아래 설명을 참고한다 |

마지막 줄은 사람이 고치지 않아도 판정이 달라질 수 있는 자리라 따로 설명한다. 편집기가 저장할 때 Markdown을 다시 포맷하면 경계 아래만 고쳤더라도 위쪽 관리 영역의 글자가 함께 바뀌고, agctx는 바이트로 비교하므로 사람이 하지 않은 충돌이 생긴다. 그래서 agctx는 관리 영역에 쓰는 내용을 흔한 포매터가 만들어 내는 형태로 맞춰 둔다. 순서 없는 목록 기호는 `-`만 쓰고, 제목 뒤에는 빈 줄을 두며, 줄 끝 공백과 연속된 빈 줄을 남기지 않는다. 이 형태를 지키는지는 저장소 평가가 검사하므로 새 템플릿이 들어와도 어긋난 채로 배포되지 않는다.

다만 문단을 정해진 너비로 다시 접는 설정을 쓰면 관리 영역의 문단이 통째로 다시 접히므로 충돌을 피할 수 없다([근거](../references.md#포매터가-관리-영역을-바꾸는-범위)). 그때는 적용된 파일을 포맷 대상에서 빼거나 `profile resolve`로 푼다.
