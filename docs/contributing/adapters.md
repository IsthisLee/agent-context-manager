...
<!-- agctx-doc-sources-sha256: d04ded58784ed95ddfc4d2a83c5e97a4a05155a5ed635807773ac5a5642f34a5 -->
...
| 2 | 에이전트가 읽는 파일의 템플릿을 만들고 적용 대상에 더한다. 관리 블록은 `<!-- agctx:managed:start -->`로 감싸고, 파일 첫 줄의 frontmatter로 규칙을 켜는 에이전트면 frontmatter를 블록 밖 맨 앞에 둔다 | `templates/`, `src/project/plan.ts`의 `POINTER_TEMPLATES`<!--s:11ddc140c26b-->(연결 파일마다 그 파일을 받는 에이전트를 함께 적는다) |
| 3 | 한 폴더에서 시작한 에이전트가 읽는 파일을 판정하는 함수를 더한다 | `src/shared/agents.ts`의 `AGENT_IDS`<!--s:dd9564bc3813-->와 `src/explain.ts`의 `explain<에이전트>`, 이유 문구 `explain.reason.*`. `AGENT_IDS`에 더하면 `profile apply --agent`와 `agctx.project.json`의 `agents`도 그 이름을 받는다 |
| 4 | 전달 증거를 읽는다. 세션 기록이 있으면 판독기를, 없으면 도구를 끄고 한 번만 실행하는 probe 명령을 더한다 | `src/verify/evidence.ts`, `src/verify/probe.ts`의 `commandFor`<!--s:ee309b774ac6--> |
