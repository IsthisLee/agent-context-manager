...
- 전역 옵션 분리와 명령 찾기: `src/commands/cli.ts`의 `main`·`run`, `src/commands/args.ts`의 `stripFlag`
- 명령 조회와 오타 제안: `src/commands/registry.ts`의 `findCommand`·`suggestCommands`·`usageLine`, `src/commands/cli.ts`의 `unknownCommand`

## 2. 로케일 해석과 저장

같은 명령이 사람에게는 고른 언어로, 자동화에는 예측 가능한 언어로 보이도록 우선순위를 한 함수에 고정한다.

- 우선순위 판정: `src/i18n/index.ts`의 `resolveLocale`
- 실행 시 해석과 저장: `src/commands/cli.ts`의 `resolveActiveLocale`, `src/shared/home.ts`의 `saveLocale`(`config.json`)
...
- 보관함 경로: `src/shared/home.ts`의 `agctxHome`·`profileHome`
- 읽기·검증: `src/profile/store.ts`의 `getProfiles`·`readProfile`·`validateProfileName`·`isValidProfileMetadata`
...
- 생성: `src/profile/store.ts`의 `createProfile`
- TUI 흐름: `src/tui/profile.ts`의 `createProfileTui`
...
- 항목 정의와 기록: `src/profile/setup.ts`의 `guidanceDefaults`·`setupProfile`·`GUIDANCE_KEYS`
...
- 공통 처리기: `src/commands/handlers.ts`의 `applyOrSync`
- 계획 수립: `src/profile/apply.ts`의 `planFor`, `src/project/plan.ts`의 `planProject`
- 계획 출력: `src/profile/apply.ts`의 `printPlan`
...
- 영역 분리: `src/project/analyzer.ts`의 `EXTENSION_HEADER`와 영역 판정 함수
- 병합·hash 기록: `src/project/plan.ts`의 `managedRegion`·`regionHash`
- 줄 끝 정규화: `src/shared/fs-utils.ts`의 `toLf`
...
- 기록된 프로필 읽기: `src/profile/apply.ts`의 `boundProfile`
- 처리기: `src/commands/handlers.ts`의 `applyOrSync`
- 지키는 평가: `evals/profile.test.ts`
...
- 대상·부모 경로 검사: `src/shared/fs-utils.ts`의 `assertSafeTextTarget`
...
- 계획 출력: `src/profile/apply.ts`의 `printPlan`
- 사람용·기계용 출력 분리: `src/commands/output.ts`의 `say`·`warn`
- 종료 코드: `src/shared/errors.ts`의 `EXIT`·`worstExitCode`·`CliError`
...
- 메인 화면: `src/tui/main.ts`의 `mainTui`·`MAIN_MENU_ENTRIES`·`MAIN_ACTIONS`
- 프로필 화면: `src/tui/profile.ts`의 `runTuiStep`·`PROFILE_MENU_COMMANDS`·`MENU_ACTIONS`·`pinPrompt`·`withConflictRecovery`
- 저장소 화면: `src/tui/repository.ts`의 `REPOS_MENU_COMMANDS`
- 명령 실행: `src/tui/commands.ts`의 `commandTokens`·`runFromTui`
- 취소 처리: `src/tui/cancel.ts`의 `cancelled`
...
- 등록부와 항목 형식: `src/commands/registry.ts`의 `COMMANDS`·`CommandSpec`·`agentPolicy`
- 옵션·인자 검사: `src/commands/options.ts`의 `checkArguments`
...
- 충돌 수집과 base 판정: `src/project/plan.ts`의 `planProject`·`knownBase`
- 충돌 표시: `src/profile/apply.ts`의 `conflictError`·`printConflicts`, `src/project/conflicts.ts`의 `formatDiff`·`baseFilePath`
- 복구 명령: `src/profile/resolve.ts`의 `resolveProject`·`mergeWithEditor`·`withBaseRegion`, `src/project/merge-editor.ts`의 `mergeInVsCode`
- TUI 복구: `src/tui/profile.ts`의 `resolveProjectTui`
...
- git 실행: `src/shared/git.ts`의 `git`·`isGitRoot`·`isRemoteFailure`·`resolveRemoteLocation`·`sanitizeRemoteUrl`
- 명령: `src/profile/git-profile.ts`의 `profileGitState`·`cloneProfile`·`pullProfile`·`planPush`·`pushProfile`·`connectProfile`
...
- 버전 결정과 기록: `src/profile/apply.ts`의 `profileVersion`
...
- 판정: `src/check.ts`의 `checkProject`·`CheckReport`
...
- 검출과 설명: `src/shared/hidden-chars.ts`의 `findHiddenCharacters`·`describeHiddenCharacters`
- 검사 지점: `src/profile/git-profile.ts`의 `assertNoHiddenCharacters`, `src/profile/apply.ts`의 `planFor`, `src/check.ts`의 `checkProject`
...
- 목록: `src/repos/registry.ts`의 `recordRepo`·`readRepos`·`pruneRepos`·`repoKey`(`$AGCTX_HOME/repos.json`)
- 상태: `src/repos/status.ts`의 `reposStatus`
- 동기화: `src/repos/sync.ts`의 `planReposSync`·`uncommittedManagedFiles`·`applyReposSync`
- PR: `src/repos/pr.ts`의 `prepareReposPrs`·`openPullRequests`·`worktreeFor`·`cloneFor`·`planTarget`·`gh`
...
- 판정: `src/explain.ts`의 `explainPath`·`chain`·`claudeImports`·`frontmatter`·`UNSUPPORTED`
- 폴더 탐색: `src/shared/scan.ts`의 `filesBelow`
...
- 확인: `src/verify/index.ts`의 `verifyPath`·`judged`·`fromSessionLog`
- 증거 판독: `src/verify/evidence.ts`의 `codexReceived`
- probe 실행: `src/verify/probe.ts`의 `probeAgent`·`commandFor`·`run`
...
- 스킬 목록과 생성: `tools/generate-skills.ts`의 `SKILLS`·`commandList`·`renderSkill`
...
- APM 판정: `src/project/apm.ts`의 `apmRegenerates`
- 연결 파일: `src/project/links.ts`의 `nestedAgentsFiles`·`gitListed`·`personLink`·`linksTo`
- 폴더 탐색: `src/shared/scan.ts`의 `filesBelow`
