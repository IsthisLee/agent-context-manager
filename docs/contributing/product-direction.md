...
사용자 문서는 처음 시작·목적별 가이드·개념·레퍼런스로, 기여자 문서는 `docs/contributing/`으로 나눈다. 문서별 책임의 정본은 [문서 안내](../README.md)의 "문서별 책임" 절이고, 내용을 어느 문서에 둘지 판정하는 기준은 루트 `AGENTS.md`의 배치 표에 있다. 진행 중인 작업과 다음 세션이 이어받을 순서는 루트 `PROGRESS.md`에 두되, 결정·근거는 쓰지 않고 정본 문서로 링크만 한다.
