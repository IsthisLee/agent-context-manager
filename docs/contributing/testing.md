...
<!-- agctx-doc-sources-sha256: a8d936ffc9f343a890cee0eec8aa4a0d10ee0ad3265a381eb9d8784fa5927428 -->
...
- 빠른 시작의 명령 예시는 `evals/doc-examples.test.ts`가, 가이드와 개념 문서에서 `<!-- agctx-example: … -->`를 붙인 예시는 `evals/doc-scenarios.test.ts`가 실제 출력과 대조한다. 명령 출력을 바꿨다면 문서의 예시를 실제 출력으로 고친다. 새 예시가 새 상태를 요구하면 `evals/doc-scenarios.test.ts`의 고정물을 더한다.
