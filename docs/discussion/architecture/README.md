...
| 17 | [에이전트 스킬을 agctx 명령으로 설치하기](topics/skill-install.md) | High | 5 | CLI 패키지에 든 스킬을 `agctx install`이 설치된 에이전트의 스킬 폴더에 복사하고, CLI와 버전이 어긋나면 모든 명령이 알림 | Proposed |
...
flowchart LR
  ...
  class S1,S2,S3,S4,S7,S8,S11,S15 done
  class S5,S6,S12 doing
  class S9,S10,S13,S14,S17 todo
