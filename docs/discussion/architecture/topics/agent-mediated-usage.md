...
| 항목 | 내용 |
| --- | --- |
| 대상 계층 | 사용자·AI 에이전트·Agentic CLI/TUI·프로필·대상 프로젝트 |
...
| 항목 | 내용 |
| --- | --- |
| 선행 작업 | 현재 CLI 명령·TUI·`agt profile list`의 기능 범위와 파일 소유권 계약 |
...
| 사용자 | 권장 경로 | 역할 |
| --- | --- | --- |
| 사람 | `agt` 또는 `agentic`의 TUI, 필요하면 `agt profile list` 관리 메뉴 | 프로필 선택, 지침 설정, 프로젝트 선택, 변경 승인 |
...
예를 들어 사용자가 “회사 프로필을 이 프로젝트에 적용해줘”라고 요청하면 에이전트는 회사 프로필의 정확한 이름과 대상 프로젝트 경로를 확인한 뒤, 적용 결과를 검토할 수 있는 명령으로 실행해야 한다. 프로필이나 경로를 추측해 조용히 선택하는 것은 Agentic의 책임 경계가 아니다.
...
sequenceDiagram
  ...
  A->>C: agt profile list
  C-->>A: 프로필 목록
  A->>U: 프로필 이름과 대상 경로 확인 요청
  U-->>A: company, 현재 저장소
  A->>C: agt profile apply company . --dry-run
  C-->>A: create · update · unchanged 파일 계획
  A->>U: 변경 계획을 보여 주고 승인 요청
  U-->>A: 승인
  A->>C: agt profile apply company .
  C-->>A: 적용 결과
  A->>U: 바뀐 파일과 경고 보고
```

Agentic이 관여하는 부분은 CLI 호출 세 번의 입력과 출력뿐이다. 프로필 이름과 대상 경로를 확인하는 일과 변경 계획을 승인받는 일은 에이전트와 사용자 사이에서 일어나며 Agentic은 이를 대신 추론하지 않는다.
...
flowchart TD
  ...
  CLI["Agentic CLI/TUI<br/>같은 기능 계약을 TUI와 CLI로 제공 · 파일 변경과 검증 수행"]
  PR["프로필<br/>공통 지침의 정본"]
...
자연어는 사용자와 AI 에이전트 사이에서만 오간다. Agentic CLI/TUI는 명시적 인자만 받아 실행한다. 적용·동기화에서 프로필은 읽기만 하고 지정된 프로젝트에만 쓴다.

- 사용자는 자연어 목표, 프로필 선택, 대상 프로젝트, 실제 변경 승인 여부를 결정한다.
- AI 에이전트는 모호한 값을 추측하지 않고 질문하거나 확인하며, TUI 화면을 흉내 내기보다 명시적 CLI를 호출한다.
- Agentic은 지정된 대상 밖으로 나가지 않고, 프로필의 공통 지침과 프로젝트의 도메인 지침을 섞어 정본을 오염시키지 않는다.
- 프로필은 에이전트가 자연어를 이해하도록 만드는 프롬프트 저장소가 아니라 공통 개발 지침의 소유 경계다.
- 대상 프로젝트는 비즈니스 코드와 도메인 지침을 소유한다.

## 권장 방향

1. **TUI는 사람용 기본 경험으로 유지한다.** `agt` 단독 실행, `profile list`의 선택·관리 메뉴, 경로 선택기는 사람이 프로필을 탐색하고 승인하기에 적합하다.
...
7. **공통 기능은 인터페이스 동등성 계약을 따른다.** AI 호출에 CLI만 추가하더라도 같은 기능이 TUI와 `agt profile list` 관리 메뉴에서 누락되지 않는지 판단한다. 다만 비대화형 실행·JSON 출력처럼 사람 메뉴에 부적합한 표현은 공통 기능의 안전한 보조 인터페이스로 예외를 명시할 수 있다.
...
- TUI·CLI·`agt profile list`의 기능 동등성 및 비대화형 예외를 검사하는 registry 평가
