...
| 항목 | 내용 |
| --- | --- |
| 대상 계층 | 프로필 관리자·구성원, Agentic CLI/TUI, 로컬 프로필 저장소, 표준 Git 원격, 적용 대상 프로젝트 |
...
| 연관 제안 | [프로젝트 적용](project-application.md), [Agentic 관리 산출물의 안전한 동기화](managed-artifact-safety.md), [자연어 요청을 통한 Agentic 사용](agent-mediated-usage.md) |
...
1. **Git은 Profile의 공유·이력·권한을 담당하고, Agentic은 Profile과 프로젝트의 안전한 적용 경계를 담당한다.**
...
| Agentic | Profile clone·상태·안전한 갱신, 적용 revision 기록, CLI/TUI 변경 계획 제공 | 모델·세션·Git 호스팅 계정·토큰·프로젝트 코드를 관리하지 않음 |
| Git 호스트 | 원격 저장소, 인증, 권한, PR·보호 규칙, 변경 이력 | Agentic 프로젝트 적용을 자동 실행하지 않음 |
...
새 기능은 [구현 계약 및 문서 규칙](implementation-contracts.md)의 인터페이스 동등성을 따른다. 각 기능은 직접 CLI, `agt` 메인 TUI, `agt profile list`에서 Profile을 선택한 관리 메뉴에 모두 등록한다.

| 명령 | 목적 | 기본 안전 동작 |
| --- | --- | --- |
| `agt profile clone <url> [--name <name>] [--branch <branch>]` | 원격 Git Profile을 로컬 Profile 저장소에 복제 | 임시 경로에 복제·metadata 검증 후 원자적으로 등록; 프로젝트 미변경 |
| `agt profile status [<name>] [--refresh]` | 원격 URL·branch·현재 revision·수정 상태·ahead/behind 표시 | 기본은 네트워크 미접속; `--refresh`만 원격 확인 |
| `agt profile pull <name> [--dry-run]` | 추적 원격의 변경을 로컬 Profile에 반영 | 수정된 worktree는 중단, `--ff-only`만 허용, 프로젝트 미변경 |
| `agt profile push <name> [--dry-run]` | 사용자가 이미 commit한 Profile 변경을 원격에 push | 자동 stage·commit 금지; 추적 원격과 push 대상 표시 |
| `agt profile connect <name> <url> [--branch <branch>]` | 기존 Git Profile에 추적 원격을 등록 | Git 저장소·metadata·원격 연결을 검증하고, commit·push는 하지 않음 |
...
```text
~/.agentic/profiles/company/
├── .git/                     # Git이 소유하는 원격·인증 연결 정보
├── AGENTS.md                 # Profile 공통 지침 정본
└── agentic-profile.json      # 이식 가능한 Profile metadata
```

`agentic-profile.json`은 원격 URL이나 사용자별 인증 정보를 저장하지 않는다. 원격 연결과 tracking branch는 Git의 `.git/config`가 정본이다. Profile metadata에는 schema 호환성, 안정적인 Profile ID, 이름, scope, 설정을 기록한다.

프로젝트가 Git Profile을 적용하면 `agentic.project.json`에 Profile 이름과 함께 적용 당시 revision을 기록한다. 권장 형태는 다음과 같다.
...
- clone은 최종 Profile 경로가 아닌 임시 경로에서 수행하고, `agentic-profile.json`·`AGENTS.md`·이름·scope·경로 안전성을 검증한 뒤에만 등록한다.
...
- push는 이미 존재하는 commit만 전송한다. Agentic은 `git add`, `git commit`, 자동 push를 수행하지 않는다.
...
Git Profile의 Markdown 지침은 AI 에이전트의 행동에 영향을 줄 수 있는 외부 입력이다. 따라서 신뢰할 수 있는 원격만 clone하고, pull 후에는 Profile diff를 검토한 뒤 프로젝트에 적용한다. Git의 권한·보호 branch·리뷰 규칙은 Git 호스트의 책임이며 Agentic이 우회하지 않는다.
...
- merge conflict를 Agentic이 의미적으로 자동 해석·해결하는 기능
