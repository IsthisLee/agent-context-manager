...
| 항목 | 내용 |
| --- | --- |
| 대상 계층 | 패키지 유지관리자 · 기본 지침 정본(`bin/i18n.mjs`, 지침 카탈로그) · `references.md` · Agentic CLI(`apply`·`sync` 출력) · 사용자 |
...
| 권장 다음 작업 | 채택 조건과 근거 등급을 먼저 확정하고 현재 6개 문장을 그 기준으로 감사한다. 그다음 분량 예산 평가를 추가하고 문구를 구체화한다. |
...
Agentic이 배포하는 기본 지침은 사용자가 에이전트 지침을 처음 갖추는 출발점이다. 사용자는 이 문구를 그대로 쓰거나 고쳐 쓰므로, 출발점의 품질이 곧 많은 프로젝트 지침의 품질이 된다. 그래서 이 제안은 다음 원칙을 세운다.
...
현재 6개 문장을 `references.md`에 기록된 근거와 대조한 결과는 다음과 같다.
...
| --- | --- | --- | --- |
| A | 에이전트 제공사의 공식 문서와 공식 엔지니어링 글 | Claude Code best practices·memory 문서, OpenAI Codex의 AGENTS.md 문서, Anthropic 엔지니어링 글 | 단독 근거로 채택할 수 있다. |
| B | 측정 방법과 결과를 공개한 연구 | arXiv 2602.11988(컨텍스트 파일의 효과 측정) | 단독 근거로 채택할 수 있다. 반대 결과가 있으면 함께 기록한다. |
| C | 실무자 글, 다른 도구의 기본 규칙 | 블로그, 오픈소스 규칙 모음 | 보조 근거로만 쓴다. C만 있는 문장은 보류한다. |
...
- Cursor와 GitHub Copilot의 한도는 아직 조사하지 않았다.
...
경고는 `apply`·`sync` 결과의 일부이므로 CLI, TUI, `agt profile list` 관리 메뉴 세 경로에서 모두 보인다.
...
| 대안 | 동작 | 판단 |
| --- | --- | --- |
| A. 분량 초과 시 자동 분리 옵션 | Agentic이 기준을 넘는 지침을 별도 문서로 옮기고 링크나 import를 남긴다. | 채택하지 않는다. 무엇을 옮길지 고르려면 내용을 이해해야 해서 모델 호출이 필요하다([ADR 0006](../../../adr/0006-no-codebase-analysis-guidance.md)과 같은 범위 문제). import로 옮기면 컨텍스트도 줄지 않는다. |
...
| Cursor·GitHub Copilot의 분량 한도 | 공식 문서를 조사해 `references.md`에 기록한다. 확인하지 못한 한도는 경고 기준에 넣지 않는다. |
