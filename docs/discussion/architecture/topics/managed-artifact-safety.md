...
| 권장 다음 작업 | 마커가 없는 기존 파일의 기본 처리와 관리 파일 목록은 [ADR 0043](../../../adr/0043-stop-on-unmanaged-files.md)으로 확정해 구현했다. 남은 것은 마커 손상에 대한 세분화된 진단과 여러 파일 전체 롤백이다. 구현 순서는 아직 정하지 않았다. |
...
#### 구현 기록: 마커 없는 기존 파일은 멈추고 `--adopt`로 편입 (2026-09-22)

* **결정:** [ADR 0043](../../../adr/0043-stop-on-unmanaged-files.md). [제안 1](#마커-없는-파일은-자동-덮어쓰지-않음)을 기본값으로, 제안 2를 명시적 옵션(`--adopt`)과 TUI 확인으로 채택했다. 제안 3(별도 파일)은 채택하지 않았다.
* **구현:** 관리하지 않는 파일의 판정은 `src/project/plan.ts`의 `planProject`가 한다. 기록(`managedHashes`)이 없고, 비어 있지 않고, agctx 표지가 없는 기존 파일을 계획의 `unmanaged`에 모은다. `AGENTS.md`의 표지 판정은 `src/project/analyzer.ts`의 `hasAgctxAgentsMarker`다. `apply`·`sync`는 `src/profile/apply.ts`의 `unmanagedError`로 종료 코드 2와 `--adopt`를 붙인 명령을 안내하고, `repos sync`·`repos pr`은 그 저장소를 `conflict`로 보고한다. TUI는 `src/tui/profile.ts`의 `recoveryFor`로 편입을 묻고 같은 명령을 `--adopt`로 다시 실행한다.
* **평가:** `evals/unmanaged-files.test.ts` 9개. 사람이 쓴 `CLAUDE.md`와 표지 없는 `AGENTS.md`에서 아무것도 쓰지 않고 멈추는지, `--dry-run`의 `unmanaged` 표시와 `--json` 오류 코드, `--adopt`가 기존 내용을 남기고 다음 `sync`부터 묻지 않는지, 빈 파일과 agctx 표지가 있는 파일, `sync --adopt`와 `repos sync`, TUI의 복구 판정과 `--adopt` 전달, 충돌과 함께 있을 때의 `resolve`, `check`의 판정, 멈춘 파일에 맞춘 안내와 `--json`의 `details`를 검사한다. 가상 터미널로 TUI의 편입 질문을 실제로 조작해 확인했다.
* **검수 반영:** 새 컨텍스트의 검수에서, 관리 영역 충돌과 표지 없는 파일이 함께 있으면 충돌 안내를 따라 실행한 `profile resolve`가 편입 확인 없이 사람이 쓴 파일에 관리 블록을 쓰는 경로가 드러났다. `resolve`도 같은 판정으로 멈추고 `--adopt`를 받게 했고(TUI는 편입을 묻는다), `check`가 그런 파일을 `sync`와 같이 충돌(2)로 보고하게 했다. 안내는 멈춘 파일에 맞췄다. `CLAUDE.md`·규칙 파일은 그 에이전트를 뺀 `--agent` 명령을 함께 보여 주고, 심볼릭 링크(`CLAUDE.md -> AGENTS.md`)에는 편입을 권하지 않는다. `--json` 오류는 멈춘 파일을 `details`에 담는다.
* **관리 파일 목록:** `managedHashes`의 키를 agctx가 관리하는 파일 목록으로 두고 새 필드는 더하지 않았다. [메타데이터 추적](#메타데이터-추적)이 제안한 관리 포맷 버전·마지막 적용 버전은 agctx를 올릴 때마다 모든 저장소의 `agctx.project.json`을 바꿔 PR을 만들기 때문이다.
* **계획과 달라진 점:** 기존 평가 가운데 사람이 쓴 파일이 있는 저장소에 처음 적용하던 평가(APM 표지 판정, 심볼릭 링크 사전 검사, 확장 영역이 없는 기존 `AGENTS.md` 보존, 분량 경고)는 원래 확인하려던 동작이 편입한 뒤의 동작이므로 `--adopt`를 붙였다.
* **제약:** 마커 손상에 대한 세분화된 진단과 여러 파일 전체 롤백은 여전히 없다.
