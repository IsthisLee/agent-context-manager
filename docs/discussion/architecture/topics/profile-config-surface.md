...
현재 agctx의 프로필은 지침 텍스트만 담는다. 제품 방향의 「범위와 경계」는 "지침의 생성·설정·동기화·적용을 담당한다"고 못박고, 모델 호출·인증·세션·런타임 오케스트레이션을 비범위로 둔다. MCP 서버 설정, skills, subagents는 이 경계 어느 쪽에도 명시돼 있지 않다. 런타임 오케스트레이션은 아니지만 지침 텍스트도 아닌, 지금 프로필이 다루지 않는 설정 영역이다.
