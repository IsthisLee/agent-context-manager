...
[문서 소스 해시 게이트의 핀 범위와 승인 단위](doc-gate-pin-scope.md)를 먼저 처리한다. 이 제안은 지금의 핀 범위와 승인 단위를 전제로 하는데, 그 두 가지를 바꾸면 전제가 달라진다. 또한 여기의 PR 단위 필터는 문서 하나만 해시 줄이 바뀐 경우를 놓친다. 2026-09-18에 확인한 PR #54가 그런 경우로, 코드와 문서 31개 파일을 바꾸면서 README만 해시 줄을 갱신했다. 같은 사례를 모델 없이 잡는 문서 단위 검출이 그 제안에 있다.
