...
test('the registry covers the complete command set', () => {
  assert.deepEqual(
    ...
    ['profile.create', 'profile.list', 'profile.view', 'profile.setup', 'profile.apply', 'profile.sync', 'profile.resolve', 'profile.remove', 'profile.clone', 'profile.link', 'profile.status', 'profile.pull', 'profile.push', 'profile.connect', 'check', 'explain', 'verify', 'repos.list', 'repos.status', 'repos.sync', 'repos.pr', 'config.lang', 'help']
  );
});
