...
test('a JSON key is cut out as its value and a YAML key as its block', () => {
  ...
  assert.equal(
    jsonValue('{"scripts":{"test":"node --test"}}', 'test'),
    '"node --test"',
    'a key inside a nested object is found too'
  );
  ...
  assert.equal(
    ...
    null,
    'a citation that points at another document carries no fingerprint'
  );
});
...
test('a name that is not a declaration cannot be cut, so the gate can reject it', () => {
  ...
  assert.equal(
    symbolText(body, 'helper'),
    null,
    'a local inside a function is not a declaration this repository cites'
  );
