...
const CLAUDE_INSTRUCTION_FILES: readonly string[] = ['claude-md-or-agents-md', 'claude-md-and-agents-md', 'claude-md', 'managed-only'];
...
function claudeInstructionFiles(configDir: string): ClaudeInstructionFiles {
  let value: unknown;
  try {
    const settings = JSON.parse(read(path.join(configDir, 'settings.json'))) as { pluginConfigs?: Record<string, { options?: Record<string, unknown> }> };
  ...
  return typeof value === 'string' && CLAUDE_INSTRUCTION_FILES.includes(value) ? value as ClaudeInstructionFiles : 'claude-md-or-agents-md';
}
...
function explainClaude(collector: Collector, target: string): void {
  ...
  const launchUnlessManagedOnly = (file: string, scope: FileScope, reason: string) => {
    ...
    if (isFile(file) && !known(file)) add(collector, file, 'not-read', scope, _('explain.reason.claude.setting-managed-only'));
  };
  ...
  const shadowing = (setting === 'claude-md-or-agents-md'
    ? ancestors(target).flatMap(dir => CLAUDE_SHADOWING_NAMES.map(name => path.join(dir, name))).filter(file => !exempt.has(file) && isFile(file))
    : []).reverse();
  const readsDirectly = readsAgents && !shadowing.length;
  const agentsAtLaunch = readsDirectly
    ? ancestors(target).flatMap(dir => [path.join(dir, 'AGENTS.md'), path.join(dir, '.claude', 'AGENTS.md')]).filter(isNonEmptyFile)
    : [];
  const launchReason = setting === 'claude-md-and-agents-md' ? _('explain.reason.claude.agents-with-claude') : _('explain.reason.claude.agents-launch');
  ...
  const doesNotArrive = (file: string, shadows: string[]) => {
    if (!readsAgents) {
      const entry = add(collector, file, 'not-read', 'project', _(managedOnly ? 'explain.reason.claude.setting-managed-only' : 'explain.reason.claude.setting-claude-md'));
    ...
    const entry = add(collector, file, 'shadowed', 'project', _('explain.reason.claude.agents-shadowed', { file: display(collector.root, nearest) }));
    if (committed) collector.findings.push({ kind: 'missing', file: entry.path, message: importAdvice(entry) });
    else collector.findings.push({ kind: 'warning', file: entry.path, message: _('explain.warning.claude-local-shadow', { file: entry.path, local: display(collector.root, nearest) }) });
  };
  ...
  for (const file of filesBelow(target, ['AGENTS.md']).filter(isNonEmptyFile)) {
    ...
    const own = setting === 'claude-md-or-agents-md'
  ...
  if (agentsAtLaunch.length || nestedAgents.length) collector.findings.push({ kind: 'warning', file: null, message: _('explain.warning.claude-direct-read') });
  if (managedOnly) collector.findings.push({ kind: 'warning', file: null, message: _('explain.warning.claude-managed-only') });
}
