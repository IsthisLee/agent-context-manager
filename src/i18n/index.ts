...
const guidance: Record<Locale, GuidanceText> = {
  ko: {
    ...
    sections: {
      ...
      security: ["보안", "비밀값은 프로젝트 안의 파일(커밋 포함)과 로그에 두지 않고 비밀값 저장소처럼 프로젝트 밖에서 읽는다. 비밀값이 노출됐으면 지우는 것으로 끝내지 말고 사용자에게 알려 폐기하고 교체하게 한다. 작업에 필요 없는 도구와 권한은 쓰지 않는다. 데이터 삭제, push·배포, 자격 증명 사용, 사용자를 대신한 게시·전송·결제처럼 되돌리기 어렵거나 영향이 큰 작업은 실행 전에 사용자 승인을 받는다. 새 의존성은 레지스트리에 실제로 있는 패키지인지와 알려진 취약점을 확인하고, 추가하기 전에 사용자에게 확인받는다."],
  ...
  en: {
    ...
    sections: {
      ...
      security: ["Security", "Never put secrets in files inside the project (including commits) or in logs; read them from outside the project, such as a secret store. When a secret is exposed, do not stop at deleting it: tell the user so it can be revoked and replaced. Do not use tools or permissions the task does not need. Get the user's approval before an action that is hard to reverse or high impact, such as deleting data, pushing or deploying, using credentials, or posting, sending, or paying on the user's behalf. Check that a new dependency exists in the registry and has no known vulnerabilities, and confirm with the user before adding it."],
