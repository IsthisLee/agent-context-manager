...
* **결정론적 검증 필수:** 코드를 수정한 뒤에는 반드시 `{{VERIFY_COMMAND}}`를 실행하여 통과를 직접 확인하라. 테스트 0개는 실패다.
...
작업 중 얻은 유용한 패턴은 `agentic` 공통 코어에 반영할 수 있으나, 본 프로젝트의 고유 비즈니스 로직이나 민감한 내부 정보는 제외하고 순수 기술 패턴으로만 추상화해야 한다.
