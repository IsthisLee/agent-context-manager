...
function checkInternalAnchors(markdownFile) {
  const content = contentWithoutCodeBlocks(
  ...
  for (const match of content.matchAll(linkPattern)) {
    ...
    if (/^(https?:|mailto:|tel:)/i.test(rawTarget) || !rawTarget.includes("#"))
      continue;
    ...
    const targetFile = target
      ...
      : markdownFile;
    if (!fs.existsSync(targetFile) || path.extname(targetFile) !== ".md")
      continue;

    const headings = [
      ...fs.readFileSync(targetFile, "utf8").matchAll(/^#{1,6}\s+(.+)$/gm),
    ...
    if (!headings.includes(fragment)) {
      errors.push(
        `${path.relative(root, markdownFile)}: missing Markdown heading anchor ${rawTarget}`,
      );
    }
  }
}
...
function checkInternalLinks(markdownFile) {
  ...
  for (const match of content.matchAll(linkPattern)) {
    const rawTarget = match[1].replace(/^<|>$/g, "");
    if (rawTarget.startsWith("#") || /^(https?:|mailto:|tel:)/i.test(rawTarget))
      continue;
    ...
    if (
      ...
      errors.push(
        `${path.relative(root, markdownFile)}: missing internal link target ${rawTarget}`,
      );
    }
  }
}
...
function checkAdrs() {
  ...
  for (const adrFile of adrFiles) {
    ...
    for (const section of [
      ...
      if (!new RegExp(`^## .*(${section})`, "mi").test(content)) {
        errors.push(
          `docs/adr/${adrFile}: missing required ADR section (${section})`,
        );
      }
    }
  }
  ...
  const indexed = [...index.matchAll(/\]\(adr\/(\d{4}-[a-z0-9-]+\.md)\)/g)].map(
    (match) => match[1],
  );
  if (indexed.join("|") !== adrFiles.join("|")) {
    errors.push(
      "docs/README.md: ADR index must contain every ADR exactly once in filename order",
    );
  }
}
...
function checkDiscussionStatuses() {
  ...
  for (const name of fs
    ...
    errors.push(
      `docs/discussion/architecture/${name}: move topic documents into topics/`,
    );
  }
  ...
  for (const name of topicFiles) {
    ...
    if (!match || !allowed.has(match[1].trim())) {
      errors.push(
        `docs/discussion/architecture/topics/${name}: use an allowed **상태:** value`,
      );
      continue;
    }
    ...
    if (["Proposed", "Implementing"].includes(match[1].trim())) {
      for (const field of proposalSummaryFields) {
        if (!content.includes(`| ${field} |`)) {
          errors.push(
            `docs/discussion/architecture/topics/${name}: missing proposal summary field (${field})`,
          );
        }
      }
    }

    const occurrences = indexedTopics.filter(
      (indexedName) => indexedName === name,
    ).length;
    if (occurrences !== 1) {
      errors.push(
        `docs/discussion/architecture/README.md: ${name} must be indexed exactly once`,
      );
    }
    const indexRow = index
      .split("\n")
      .find((line) => line.includes(`](topics/${name})`));
    const indexStatus = indexRow
      ?.split("|")
      .map((cell) => cell.trim())
      .filter(Boolean)
      .at(-1);
    if (indexStatus !== match[1].trim()) {
      errors.push(
        `docs/discussion/architecture/README.md: status for ${name} must match its document`,
      );
    }
  }
  ...
  for (const indexedName of new Set(indexedTopics)) {
    if (!topicFiles.includes(indexedName)) {
      errors.push(
        `docs/discussion/architecture/README.md: index references missing topic ${indexedName}`,
      );
    }
  }
}
...
function checkReadme() {
  ...
  for (const heading of requiredHeadings) {
    if (!content.includes(heading))
      errors.push(`README.md: missing required section ${heading}`);
  }
  for (const requiredText of [
    ...
    if (!content.includes(requiredText))
      errors.push(
        `README.md: missing required product guidance ${requiredText}`,
      );
  }
}

function checkDocumentationGovernance() {
  const proposalFormat = path.join(
    root,
    "docs",
    "discussion",
    "architecture",
  ...
  for (const file of requiredReferences) {
    if (
      !fs.readFileSync(file, "utf8").includes("implementation-contracts.md")
    ) {
      errors.push(
        `${path.relative(root, file)}: must link to the canonical proposal format`,
      );
    }
  }
  ...
  for (const file of [
    ...
    if (!fs.existsSync(file))
      errors.push(
        `${path.relative(root, file)}: public repository contract file must exist`,
      );
  }
}
...
function checkChangelog() {
  ...
  if (
    ...
    !/^## \[Unreleased\]/m.test(fs.readFileSync(changelog, "utf8"))
  ) {
